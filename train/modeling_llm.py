...
class llmRotaryEmbedding(nn.Module):
    def __init__(self, dim, max_position_embeddings = 2048, base = 10000, device = None):
        ...
        # 为了支持 `torch.jit.trace` 功能，立即计算与存储的余弦和正弦缓存
    ...
    def _set_cos_sin_cache(self, seq_len, device, dtype):
        ...
        # 创建一个从 0 到最大序列长度 -1 的整数张量，与 inv_freq 具有相同的设备和数据类型
        t = torch.arange(
    ...
    def forward(
        ...
        **kwargs
        ...
        if "padding_mask" in kwargs:
            ...
            # softmax 归一化注意力权重，并转换至 float32 类型以防止数值溢出
...
class llmSdpaAttention(llmAttention):
    ...
    def forward(
        ...
        # 对输入进行线性映射得到 query、key、value 向量
        query_states = self.q_proj(hidden_states)
        key_states = self.k_proj(hidden_states)
        value_states = self.v_proj(hidden_states)
        ...
        # 使用 scaled_dot_product_attention 进行计算
...
class llmPreTrainedModel(PreTrainedModel):
    ...
    # 指定了在序列化过程中不应该被拆分的模块列表，即在模型保存与加载时保持这些模块作为一个整体
...
class llmModel(llmPreTrainedModel):
    ...
    def forward(
        ...
        if self._attn_implementation == "sdqa" and not output_attentions:
...
class llmForCausalLM(llmPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # 其中 shift_logits 变成 (batch_size * sequence_length, vocab_size) 的形式
            # shift_labels 变为 (batch_size * sequence_length) 的形式
            shift_logits = shift_logits.view(-1, self.config.vocab_size)
            shift_labels = shift_labels.view(-1)

            # Enable model parallelism
            # 确保模型并行计算时，labels 的数据存储位置与 logits 一致
    ...
    def prepare_inputs_for_generation(
        ...
        if past_key_values is not None:
            ...
            # 根据缓存情况裁剪 input_ids，只保留未处理的 token：
            ...
            # 3. 否则，认为 input_ids 中只有待处理的新 token
        ...
        # 如果 attention_mask 存在但 position_ids 不存在，则基于 attention_mask 动态创建 position_ids
        ...
        **kwargs,
        ...
        if repetition_penalty > 1.0:
            ...
            logits_processor = LogitsProcessorList(
                [OutputRepetitionPenaltyLogitsProcessor(input.size(1), presence_penalty, frequency_penalty, 1.0)]
            )
    ...
    def chat(
        ...
        if stream:
            streamer = TextIterStreamer(
                ...
                skip_special_tokens = True, use_pot = use_pot
            )
            Thread(target = self.generate, kwargs = dict(
                inputs = inputs_ids, streamer = streamer,
                generation_config = generation_config
            )).start()
        else:
            ...
            generated_ids = [
                output_ids[len(input_ids):] for input_ids, output_ids in zip(inputs_ids, generated_ids)
            ]
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if self.config.problem_type == "regression":
                # 使用均方误差损失函数
                loss_fct = MSELoss()
                # 如果 num_labels 为 1，则直接计算单输出的损失；否则，按列计算所有输出损失
                if self.num_labels == 1:
        ...
        return SequenceClassifierOutputWithPast(
            ...
            attention = transformer_outputs.attentions
        ) 
...
if __name__ == "__main__":
    ...
    model = llmForCausalLM(args_1480m)
