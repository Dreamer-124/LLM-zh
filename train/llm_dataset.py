...
class PTMDataset(Dataset):
    def __init__(self, data_path_list, max_length = 512, memmap = False):
        ...
        if memmap:
            ...
            self.data = np.memmap(
                data_path_list[0], dtype = np.dtype('unit16'), shape = (flen // max_length, max_length)
            )
        else:
            data_list = []
...
class PTMDatasetMap(Dataset):
    ...
    def __init__(self, data_path_list: list, max_length = 512):
        ...
        for idx, file_path in enumerate(data_path_list):
            ...
            # 使用内存映射读取数据
            self.data.append(
                np.memmap(file_path, dtype = np.dtype('uint16'), shape = (flen // max_length, max_length))
            )
...
class SFTDataset(Dataset):
    ...
    def preprocessing(self, example, debug = False):
        ...
        response = self.tokenizer.encode(
            assistant_txt.strip(), add_special_tokens = False, truncation = True, max_length = self.max_length
        )
...
class RMDataset(Dataset):
    ...
    def preprocessing(self, example, debug = False):
        ...
        response_j = self.tokenizer.encode(
            assistant_chosen_txt.strip(), add_special_tokens = False, 
...
def load_ppo_dataset(
    ...
    if sanity_check:
        dataset = dateset.select(range(min(len(dataset), 1000)))
    ...
    dataset_map = dataset.map.filter(
