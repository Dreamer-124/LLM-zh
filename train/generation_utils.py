...
def make_context(
    mode, tokenizer, message: List[dict],
    system: str = "You are a helpful assistant.",
    max_new_tokens: int = 0
):
    # 确定新生成的 token 数量，优先使用传入参数，否则使用模型配置中的默认值
    max_new_tokens = max_new_tokens or model.generation_config.max_new_tokens
    # 计算模型允许的最大输入长度（模型最大长度减去新生成的 token 数）
    ...
    if system_text:
        ...
        final_tokens = tokenizer.encode(
            "<|assistant|>", add_special_tokens = False, truncation = True) + nl_tokens
        ...
        for turn_query, turn_response in reversed(history):
            ...
            history_query_tokens = tokenizer.encode(
                "<|user|>\n" + turn_query.strip(), add_special_tokens = False, 
                truncation = True) + nl_tokens
            ## answer tokens
            histroy_response_tokens = tokenizer.encode(
                "<|assistant|>\n" + turn_response.strip(), add_special_tokens = False, 
                truncation = True) + nl_tokens
            ## this round tokens
            next_context_tokens = history_query_tokens + histroy_response_tokens
            ## concat
            ## 确保加入这些 token 后总长度不超过允许的最大历史长度
            ...
            try:
                ...
                if "func" in k:
                    ...
                    try:
                        ...
                        if 'sympy' in var[1]:
                            var[1] = var[1].replace(
                                'res[x]', 'res[0][0]').replace('res[y]', 'res[0][1]')
                            exec(var[1], globals())
                            ans = func()
                        pass
    ...
    def put(self, value):
        ...
        if self.skip_prompt and self.next_tokens_are_prompt:
            ...
            tokens_str = self.tokenizer.decode(
                self.tokens, skip_special_tokens = self.skip_special_tokens, errors = 'ignore')
            if self.use_pot:
...
class OutputRepetitionPenaltyLogitsProcessor(LogitsProcessor):
    ...
    def __init__(
        ...
        if not (repetition_penalties > 0):
            raise ValueError(
                f"`repetition_penalties` has to be a strictly positive float, but is {repetition_penalties}"
            )
        if not ( (frequency_penalties >= -2) and (frequency_penalties <= 2) ):
            raise ValueError(
                f"`frequency_penalties` has to be [-2, 2], but is {frequency_penalties}"
            )
        if not ( (presence_penalties >= -2) and (presence_penalties <= 2) ):
            raise ValueError(
                f"`presence_penalties` has to be [-2, 2], but is {presence_penalties}"
            )
    ...
    def _get_bin_counts_and_mask(
        ...
        bin_counts = torch.zeros(
            (num_seqs, vocab_size + 1), dtype = torch.long,
