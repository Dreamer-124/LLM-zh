...
from llm_dataset import PTMDataset
...
def main():
    ...
    gpt_conf = llmConfig(**gpt_args)
    model = llmForCausalLM(gpt_conf)
    model.to(device)
    ...
    train_ds = PTMDataset(
        data_path_list, max_length = model_args.max_position_embeddings, memmap = False
    )
