...
from modeling_llm import lmForCausalLM, llmForSequenceClassification
from llm_dataset import RMDataset
...
def main():
    parser = HfArgumentParser(m(ModelArguments, ScriptArguments, TrainingArguments))
    ...
    model = llmForSequenceClassification.from_pretrained(
        script_args.base_model_path,
        config = config,
        trust_remote_code = True
    )
