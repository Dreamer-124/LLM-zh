        ...
        for idx, file in enumerate(files):
            ...
            if file.endswith('.json'):
                ...
                for line in tqdm(lines):
                    json_obj = json.loads(line)  # 解析 json 字符串为 python 对象
                    text = json_obj['text']
                    tokens = tokenizer.encode(text, add_special_token = False)
                    tokens.append(tokenizer.special_tokens['<eos>'])
                    if len(tokens) > 5:
                        all_tokens += tokens
...
def process_baidu_baike(input_path, tokenizer):
    ...
    while True:
        ...
        text_id = tokenizer.encode(text, add_special_token = False)
